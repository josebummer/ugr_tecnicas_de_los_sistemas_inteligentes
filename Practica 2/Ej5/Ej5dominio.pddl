(define (domain belkan-domain)	       ; Comment: adding location caused fail
	...
	(:action ENTREGAR
			...
			:precondition (and (esta ?j ?l) (esta ?p ?l) (llevaMano ?ob) (not(llevaMano bikini)) (not(llevaMano zapatillas)) (< (bolsillo ?p) (tambolsillo ?p)))
			:effect (and (not(llevaMano ?ob)) (tieneobjeto ?p) (not(mano ?j)) (increase (puntos-totales) (puntos ?p ?ob)) (increase (bolsillo ?p) 1))
	)
